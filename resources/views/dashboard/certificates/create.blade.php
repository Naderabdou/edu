...
@section('title', transWord('إضافة شهاده جديد'))
                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="#">{{ transWord('إضافة شهاده جديد') }}</a>
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h2 class="card-title">{{ transWord('إضافة شهاده جديد') }}</h2>
                                </div>
                                <div class="card-body">
                                    <form class="form form-vertical" id="createCategoryForm"
                                        action="{{ route('admin.certificates.store') }}" method="POST"
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-12">
                                                <div class="form-group">
                                                    <label for="html">{{ transWord('html الشهاده') }}</label>
                                                    <textarea id="html" class="form-control " name="html" style="width: 100%; height: 300px;">{{ old('html') }}</textarea>
                                                     @error('html')
    ...
    @push('js')
        ...
        <script>
            ...
            const pond = FilePond.create((category), {
                ...
                onprocessfile: function(error, file) {
                    if (error) {
                        ...
                        imageCertificate = file.serverId;
                        localStorage.setItem('image_certificate', imageCertificate);
                    }
                }
            });
            pond.setOptions({
                    ...
                    server: {
                        ...
                        process: "{{ route('admin.tmp.uploads', ['folder' => 'certificates']) }}",
                        revert: "{{ route('admin.tmp.delete', ['folder' => 'certificates']) }}",
                        headers: {
            ...
            if (performance.navigation.type === performance.navigation.TYPE_RELOAD) {
                const uploadedFileId = localStorage.getItem('image_certificate');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        localStorage.removeItem('image_certificate');
                    },
